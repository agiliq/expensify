...
class SimpleViews(TestCase):
    def setUp(self):
        ...
        self.category = ExpenseCategory.objects.create(
            title="Foo", description="Bar", max_limit=200)
    ...
    def test_profile(self):
        '''
        Profile should redirect for not logged in users and let in for loggedin
        '''
...
class TestModels(TestCase):
    ...
    def test_emails(self):
        ...
        expense = Expense.objects.create(
            amount=100, date="2012-10-13", category=self.category,
            usr=self.user)
        self.assertEqual(len(mail.outbox), 1)
        expense.status = True
        expense.save()
